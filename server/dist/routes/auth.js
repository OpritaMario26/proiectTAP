...
const createAccessToken = (userId, role) => jwt.sign({ sub: userId, role }, env.JWT_ACCESS_SECRET, { expiresIn: '15m' });
const createRefreshToken = (userId) => jwt.sign({ sub: userId, type: 'refresh' }, env.JWT_REFRESH_SECRET, {
    expiresIn: '7d',
});
...
authRouter.post('/register', async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({
            message: 'User created, but verification email could not be sent. Configure SMTP and request verification again.',
            user: {
                id: user.id,
                email: user.email,
...
authRouter.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        const isMatch = user.refreshTokenHash === hashToken(parsed.data.refreshToken);
        if (!isMatch) {
            return res.status(401).json({ message: 'Invalid refresh token' });
        }
        const accessToken = createAccessToken(user.id, user.role);
...
authRouter.post('/request-reset', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({
            message: 'If email exists, reset token generated but email could not be sent',
            resetToken,
        });
    }
    return res.status(200).json({
