...
productsRouter.get('/', async (req, res) => {
  ...
  const { search, categoryId, brand, minPrice, maxPrice, page, pageSize, sort } = parsed.data;
...
productsRouter.delete('/:id', authenticate, requireAdmin, async (req, res) => {
  ...
  if (blockedOrderItem) {
    return res.status(409).json({
      message:
        'Produsul nu poate fi sters pentru ca exista comenzi active care il contin. Stergerea este permisa doar dupa finalizarea sau anularea acestor comenzi.',
    });
  }
