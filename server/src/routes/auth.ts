...
authRouter.post('/register', async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      message:
        'User created, but verification email could not be sent. Configure SMTP and request verification again.',
      user: {
        id: user.id,
        email: user.email,
...
authRouter.post('/request-reset', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message:
        'If email exists, reset token generated but email could not be sent',
      resetToken,
    });
  }
